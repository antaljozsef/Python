...
while True:
    ...
    if ids is not None and len(ids) > 1:
        ...
        for i in range(len(ids)):
            ...
            marker_width_mm = (
                marker_width * camera_to_marker_distance_mm / frame.shape[1]
            )
            marker_height_mm = (
                marker_height * camera_to_marker_distance_mm / frame.shape[0]
            )
        ...
        marker_widths = (
            np.abs(all_corners[:, 0, 0, 0] - all_corners[:, 0, 1, 0])
            * camera_to_marker_distance_mm
            / frame.shape[1]
        )
        marker_heights = (
            np.abs(all_corners[:, 0, 0, 1] - all_corners[:, 0, 3, 1])
            * camera_to_marker_distance_mm
            / frame.shape[0]
        )
        ...
        for i in range(len(all_ids)):
            ...
            ret, chessboard_corners = cv2.findChessboardCorners(
